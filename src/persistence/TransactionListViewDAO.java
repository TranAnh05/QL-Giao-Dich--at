...
public class TransactionListViewDAO implements TransactionGateway, TotalTransactionGateway {
    ...
    public List<TransactionDTO> getTransactionsByType(String type) throws SQLException {
        List<TransactionDTO> list = new ArrayList<>();
        String sql = "SELECT id, date, unitPrice, area, transactionType, landType, houseType, address FROM transaction WHERE transactionType = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
